...
- Create an **Environment Variable** named **OPENGL_PATH** (don't ask) with the value being the path to the newly created folder.  
  ...
  - On Windows: searct for "environment variables" in the taskbar and you should manage to open a menu where you can add environment variables. Add **OPENGL_PATH** and give it the path to your folder.

- Now open a terminal/powershell/cmd in this project's path and run `cmake -B build`, then `cmake --build build --config Release -j4`. Now you should see your compiled binary in the **"bin"** folder: run with `./bin/Juliet` on MacOS/Linux, and `./bin/Release/Juliet.exe` on Windows.
