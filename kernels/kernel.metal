...
ScatterResult scatterEmmisive(thread PCG32& rng, thread const Ray& ray, thread const HitRecord& hit, thread const float data[MATERIAL_DATA_SIZE]) {
    ScatterResult res;
    res.albedo = float3(data[0], data[1], data[2]);
    res.scattered = false;
    return res;
...
ScatterResult scatterMaterial(thread PCG32& rng, thread const Ray& ray, thread const HitRecord& hit, thread const Material& material) {
    ...
    case EMMISIVE: {
        return scatterEmmisive(rng, ray, hit, material.data);
    }
